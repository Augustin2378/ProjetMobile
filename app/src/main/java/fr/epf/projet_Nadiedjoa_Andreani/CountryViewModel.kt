...
class CountryViewModel(context: Context, private val restCountryApi: RestCountryApi) : ViewModel() {
        ...
        fun filterCountries(query: String, onCountriesFiltered: (List<Country>) -> Unit) {
                viewModelScope.launch {
                        try {
                                val countryList = restCountryApi.getCountryByName(query)
                                onCountriesFiltered(countryList)
                        } catch (e: Exception) {
                                Log.e("CountryViewModel", "Error filtering countries: ${e.message}", e)
